...
from Widgets.CustomBehaviors.skills.ranger.comfort_animal_utility import ComfortAnimalUtility
from Widgets.CustomBehaviors.skills.ranger.distracting_shot_utility import DistractingShotUtility
from Widgets.CustomBehaviors.skills.ranger.never_rampage_alone_utility import NeverRampageAloneUtility
...
class RangerTaoVolley_UtilitySkillBar(CustomBehaviorBaseUtility):
    ...
    def __init__(self):
        ...
        # core
        self.together_as_one_utility: CustomSkillUtilityBase = TogetherAsOneUtility(event_bus=self.event_bus, current_build=in_game_build, mana_required_to_cast=4, score_definition=ScoreStaticDefinition(95))

        self.savage_shot_utility: CustomSkillUtilityBase = SavageShotUtility(event_bus=self.event_bus, current_build=in_game_build, mana_required_to_cast=4, score_definition=ScoreStaticDefinition(91))
        self.distracting_shot_utility: CustomSkillUtilityBase = DistractingShotUtility(event_bus=self.event_bus, current_build=in_game_build, mana_required_to_cast=2, score_definition=ScoreStaticDefinition(92))

        self.volley_utility: CustomSkillUtilityBase = RawAoeAttackUtility(event_bus=self.event_bus, skill=CustomSkill("Volley"), mana_required_to_cast=2, current_build=in_game_build, score_definition=ScorePerAgentQuantityDefinition(lambda enemy_qte: 70 if enemy_qte >= 3 else 65 if enemy_qte <= 2 else 25))

        #optional
        self.never_rampage_alone_utility: CustomSkillUtilityBase = NeverRampageAloneUtility(event_bus=self.event_bus, current_build=in_game_build, mana_required_to_cast=6, score_definition=ScoreStaticDefinition(80), allowed_states=[BehaviorState.IN_AGGRO, BehaviorState.CLOSE_TO_AGGRO])
        ...
        self.sundering_attack_utility: CustomSkillUtilityBase = RawSimpleAttackUtility(event_bus=self.event_bus, skill=CustomSkill("Sundering_Attack"), current_build=in_game_build, mana_required_to_cast=4, score_definition=ScoreStaticDefinition(60))
        self.comfort_animal_utility: CustomSkillUtilityBase = ComfortAnimalUtility(event_bus=self.event_bus, current_build=in_game_build, mana_required_to_cast=2, score_definition=ScoreStaticDefinition(90), allowed_states=[BehaviorState.IDLE, BehaviorState.CLOSE_TO_AGGRO, BehaviorState.FAR_FROM_AGGRO])
        ...
        self.signet_of_return_utility: CustomSkillUtilityBase = GenericResurrectionUtility(event_bus=self.event_bus, skill=CustomSkill("Signet_of_Return"), current_build=in_game_build,score_definition=ScoreStaticDefinition(99))
