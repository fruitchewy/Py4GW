...
class TestGeometry:
    ...
    def test_count_inverted_top(self, map_dump: JsonDumpData):
        """Report traps with XTL > XTR (inverted top edge)."""
        inverted = [t.id for t in map_dump.trapezoids if t.XTL > t.XTR]
        print(f"Traps with XTL > XTR: {len(inverted)} / {len(map_dump.trapezoids)}")

    def test_count_inverted_bottom(self, map_dump: JsonDumpData):
        """Report traps with XBL > XBR (inverted bottom edge)."""
        inverted = [t.id for t in map_dump.trapezoids if t.XBL > t.XBR]
        print(f"Traps with XBL > XBR: {len(inverted)} / {len(map_dump.trapezoids)}")
...
class TestTestPoints:
    def test_has_test_points(self, map_dump: JsonDumpData):
        if not map_dump.test_points:
            pytest.skip("No test points in this dump")
        assert len(map_dump.test_points) > 0
