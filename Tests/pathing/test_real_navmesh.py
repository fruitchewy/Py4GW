...
class TestTestPointLookup:
    ...
    def test_test_points_resolve(self, map_dump: JsonDumpData, map_navmesh: NavMesh):
        """Each test point should find a trapezoid."""
        if not map_dump.test_points:
            pytest.skip("No test points in this dump")
        for tp in map_dump.test_points:
            found = map_navmesh.find_trapezoid_id_by_coord(tp.coords)
            assert found is not None, (
                f"Test point '{tp.name}' at {tp.coords} not found in any trapezoid"
            )

    def test_test_points_match_recorded_trap(self, map_dump: JsonDumpData, map_navmesh: NavMesh):
        """Test points should resolve to the trapezoid ID recorded during capture.

        The JSON dump records which trap each test point was in at capture time.
        After global re-indexing the IDs may differ, so we check the geometry
        instead: the found trapezoid should overlap with the original trap's position.
        """
        if not map_dump.test_points:
            pytest.skip("No test points in this dump")
        for tp in map_dump.test_points:
            found = map_navmesh.find_trapezoid_id_by_coord(tp.coords)
            if found is None:
                pytest.skip(f"Test point '{tp.name}' not resolved (may be inverted trap)")
...
class TestAStar:
    def test_path_between_all_markers(self, map_dump: JsonDumpData, map_navmesh: NavMesh):
        ...
        if not pairs:
            pytest.skip("Need at least 2 test points")
